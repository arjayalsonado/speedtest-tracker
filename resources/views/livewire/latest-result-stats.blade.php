<div wire:poll.60s>
    @if ($this->latestResults->isNotEmpty())
        <div class="space-y-6 latest-result-stats">
            @foreach ($this->latestResults as $item)
                @php
                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                <h2 class="flex items-center gap-x-2 text-base md:text-lg font-semibold text-zinc-900 dark:text-zinc-100">
                                    ...
                                    @filled($profileName)
                                        {{ __('general.isp_profile_last_results', ['profile' => $profileName]) }}
                                    @else
                                        {{ __('general.last_results') }}
                                    @endfilled
                                </h2>

                                <p class="mt-1 text-sm font-medium text-zinc-600 dark:text-zinc-400">{{ $result->created_at->timezone(config('app.display_timezone'))->format(config('app.datetime_format')) }}</p>
                            </div>
